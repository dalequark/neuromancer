...
def get_prediction(trialNum):
    try:
        ...
        if bin_data.shape[0] == 0:
            ...
            place_prob = probs[1]
            if trialType == PLACES:
                diff_prob = sigmoid(place_prob - face_prob, 0.6,10,.05, 0.30)
            else:
                diff_prob = sigmoid(face_prob - place_prob, 0.6,10,.05, 0.30)
            ...
            eegDataQueue = eegDataQueue[idxshigh[0][0]:]
